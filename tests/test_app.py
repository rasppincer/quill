...
class TestDebugPrompt:
    ...
    def test_debug_prompt_content_stage_shows_single_call(self, client, sample_piece_with_review, tmp_output, monkeypatch):
        """Content stage debug shows single_call with separator."""
        monkeypatch.setattr("quill.piece.DEFAULT_OUTPUT_DIR", tmp_output)
        ...
        assert "dad40ab6" in data["separator"]
        assert data["single_call"]["char_count"] > 0
